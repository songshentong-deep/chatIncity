...
build:
	...
	cd backend && go build -o bin/upload-service ./services/upload-service
	@echo "构建Flutter应用..."
...
backend-dev:
	...
	@echo "启动图片服务 (端口8009)..."
	cd backend && go run ./services/upload-service &
	@echo "启动动态服务 (端口8010)..."
	cd backend && go run ./services/moment-service &
	@echo "启动新闻服务 (端口8011)..."
	cd backend && go run ./services/news-service &
	@echo "所有后端服务已启动！"
...
# 重启指定端口的服务
# 用法: make restart-service PORT=8009
restart-service:
	@if [ -z "$(PORT)" ]; then \
		echo "❌ 请指定端口号，例如: make restart-service PORT=8009"; \
		exit 1; \
	fi
	@echo "🔄 重启端口 $(PORT) 的服务..."
	@./scripts/restart-service.sh $(PORT)

# 热更新模式启动后端服务
hot-reload:
	@echo "启动热更新模式..."
	@cd backend && chmod +x scripts/hot-reload.sh && ./scripts/hot-reload.sh
