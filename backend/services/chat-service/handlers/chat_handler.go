...
func (h *ChatHandler) CreateChatRoom(c *gin.Context) {
	...
	room, err := h.chatService.CreateChatRoom(&req, userID.(string))
	if err != nil {
		c.JSON(http.StatusInternalServerError, service.ErrorResponse(500, err.Error()))
		return
	}
...
// 获取或创建私聊
func (h *ChatHandler) GetOrCreatePrivateChat(c *gin.Context) {
	userID, exists := c.Get("user_id")
	if !exists {
		c.JSON(http.StatusUnauthorized, service.ErrorResponse(401, "未授权"))
		return
	...
	var req service.GetOrCreatePrivateChatRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, service.ErrorResponse(400, "请求参数错误: "+err.Error()))
		return
	}

	room, err := h.chatService.GetOrCreatePrivateChat(userID.(string), req.FriendID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, service.ErrorResponse(500, err.Error()))
		return
	}
...
func (h *ChatHandler) GetGroupMembers(c *gin.Context) {
	...
	members, err := h.chatService.GetGroupMembers(userID.(string), chatID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, service.ErrorResponse(500, err.Error()))
		return
	}

	c.JSON(http.StatusOK, service.SuccessResponse(members))
}